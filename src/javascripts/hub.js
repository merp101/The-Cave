import {changeText, hide, show} from "main.js"

var npcList = ["nurse", "blacksmith"];
var npcs = {
  //dialogue comments format: (yes/no)(line it's responding to(/other line if it's shared))("e" if it ends the dialogue)("f" if you failed the dialogue)
  nurse: {
    pos: 24, //-5 (to line number == array index)
    dialogue: [
      "Oh! Hello there! You look a bit dirty... and naked... Well, I'll fix you <em>right</em> up. Come inside my heart shack and find out!",
    ...
    secondaryDialogue: [
      "Oh, hello! Welcome back! Would you like to browse my wares?",
      "Oh, then would you like to come inside my heart shack instead?"
    ],
    dialoguetrees: {
      //comment 's' if it asks for a shop
      0: { yes: 14, no: 1 },
      1: { yes: 2, no: 7 },
      2: { yes: 3, no: 4 },
      3: { yes: 6, no: 5 }, //s
  ...
  blacksmith: {
    pos: 46,
    dialogue: [
      "Hi! I'm the blacksmith. Would you like to buy a this wooden sword? Because it is made of wood it is only 1 gold! <br> (It looks like a toy.)",
    ...
    secondaryDialogue: [
      "Oh! Hello again! Care to stop and talk to this lonely old geezer?",
  ...
  if (npc.dialoguetrees[npc.dialoguecycle] === undefined) {
    ...
    if (npc === "nurse") {
      if (
        ...
        npc.dialoguecycle === 4 ||
        npc.dialoguecycle === 8 ||
        npc.dialoguecycle === 16 ||
        npc.dialoguecycle === 18
      ) {
...
export { npcs, npcList, cycleNPCDialogue };
