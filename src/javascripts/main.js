import { npcs, npcList, cycleNPCDialogue } from "npc.js";
import { game, char, buffOrder, skillOrder } from "config.js";
import { invArt } from "art.js";
import {
	finishQuest,
	drawWorld,
	currentLevel,
...
function tick(letter = 0) {
	...
	function move() {
		switch (letter) {
			case "a":
				if (char.pos.x > 1) char.pos.x -= 1;
				break;
			case "d":
				if (
					char.pos.x <
					element("world-" + (char.pos.y + 1)).innerHTML.length - 3
				)
					char.pos.x += 1;
				break;
			default:
	...
	if (letter === "w") {
		...
		for (i = 0; i < Object.keys(npcs).length; i++) {
			npc = npcs[npcList[i]];
			if (
				char.pos.x + 2 === npc.pos ||
				char.pos.x + 1 === npc.pos ||
				char.pos.x === npc.pos ||
				char.pos.x - 1 === npc.pos ||
				char.pos.x - 2 === npc.pos
			) {
	...
	if (!game.conditions.fighting) {
		...
		if (currentLevel !== "hub") {
			if (levelEnemies[0] !== undefined) {
				if (
					(char.pos.x - 1 === Number(levelEnemies[0].charAt(0)) ||
						char.pos.x + 2 === Number(levelEnemies[0].charAt(0))) &&
					char.pos.y === Number(levelEnemies[0].charAt(2))
				) {
		...
		if (currentTab === "fight") {
			if (worldDrawn) {
				switch (x) {
					...
					//attacks
					case 49:
						fight(numToLettersAtk[0]);
						break; // 1
					case 50:
						fight(numToLettersAtk[1]);
						break; // 2
					case 51:
						fight(numToLettersAtk[2]);
						break; // 3
					case 52:
						fight(numToLettersAtk[3]);
						break; // 4
					default:
						break;
				}
	...
	if (game.conditions.fighting) {
		...
		for (i = 0; i < skillOrder.length; i++) {
			...
			if (game.stats.skills.includes(skill)) {
				//if that skill is unlocked, then
				show(skill + "atk"); //show the element for it
			} else {
				hide(skill + "atk");
			}
		}
		for (i = 0; i < buffOrder.length; i++) {
			//buffs
		...
		let count = 0;
		for (i = 0; i < buffOrder.length; i++) {
			//hide the "buff" thing if there are no buffs unlocked (not for skills cuz you always have "basic" skill unlocked
		...
		if (
			currentEnemy.name.charAt(0) === "a" ||
			currentEnemy.name.charAt(0) === "e" ||
			currentEnemy.name.charAt(0) === "i" ||
			currentEnemy.name.charAt(0) === "o" ||
			currentEnemy.name.charAt(0) === "u" ||
			currentEnemy.name.charAt(0) === "A" ||
			currentEnemy.name.charAt(0) === "E" ||
			currentEnemy.name.charAt(0) === "I" ||
			currentEnemy.name.charAt(0) === "O" ||
			...
			changeText("enemyname", "a " + currentEnemy.name);
		}
...
function init() {
	...
	if (game.items.equips.weapon.name === "none")
		setPlayerItem("fists", "fists", 1);
}
...
export { changeTab, show, hide, element, display };
