import { game, loot, bubbleSort, i } from "config.js";
...
function getTotalLoot() {
  //on mission complete, gain all the stuff
  ...
  for (let i = 0; i < itemsGained.length; i++) {
    ...
    switch (type) {
      case "normal":
        game.inventory.normal.push(itemsGained[i]);
        break;
      case "special":
        game.inventory.special.push(itemsGained[i]);
        break;
      case "consumable":
        game.inventory.consumable.push(itemsGained[i]);
        break;
      default:
        break;
    }
...
function getEnemyLoot() {
  ...
  if (currentEnemy !== undefined) {
    ...
    for (i = 0; i < loot[currentLevel].items.length; i++) {
      combo.push([
        loot[currentLevel].items[i].name,
        Math.floor(
          Math.pow(
            loot[currentLevel].items[i].chance,
            Math.floor(Math.pow(rarity, 0.2))
          )
        )
      ]);
    ...
    if (element("message" + enemiesDefeated) === undefined) {
      ...
      changeText("message" + enemiesDefeated, singleMessage);
    }
  }
}

export { getEnemyLoot, getTotalLoot };
