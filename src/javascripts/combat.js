...
import { maps, generalArt } from "art.js";
import { show, hide, changeText, element, display } from "main.js";
...
function Enemy(level, hpmax, atk, def, spd) {
	...
	this.rarity = rarity;
	if (rarity === 1) {
		names = ["goblin", "imp", "spider"];
	} else if (rarity === 2) {
		names = ["hobgoblin", "fiery imp"];
	} else if (rarity === 3) {
		names = ["orc", "demon"];
	} else if (rarity === 4) {
		names = ["treasure"];
...
function setEnemies(difficulty = 0, level = currentLevel) {
	...
	game.enemies.hp.max =
		Math.round(Math.pow(game.stats.hp.max, 0.25) + 1) *
			Math.pow(difficulty, 2) +
		1;
...
function fight(attack, buffs = []) {
	if (game.stats.skills.includes(attack) && game.conditions.fighting) {
		damage =
			game.stats.atk +
			game.items.equips.weapon.atk *
				consts.weaponTypeModifiers[game.items.equips.weapon.type].dmg;
		var dmgMult;
		...
		currentEnemy.hp.current -=
			damage * dmgMult - Math.floor(currentEnemy.def / 2); //can be changed
		switch (game.items.equips.weapon.type) {
			...
			default:
				console.log("No weapon type defined!");
				break;
		}
		setTimeout(function() {
			changeText(
			...
			consts.weaponTypeModifiers[game.items.equips.weapon.type].speed);
	}
}
...
function drawWorld(level) {
	if (!worldDrawn) {
		...
		for (let y = maps[level].length; y > 0; y--) {
			...
			if (
				element("world-" + (maps[level].length - y + 1)) !== undefined
			) {
				//if the element's already there
				changeText("world-" + (maps[level].length - y + 1), str);
				if (
					element("world-" + (maps[level].length - y + 1)).style
						.display === "none"
				) {
				...
				if (map.childNodes[0] === undefined) {
					//if it's the first element it's making (for the entire game)
		...
		for (
			i = 1;
			i < maps[currentLevel].length - maps[level].length + 1;
			i++
		) {
	...
	if (!enemiesDrawn) {
		if (enemiesSet) {
			for (i = 0; i < maps[currentLevel + "EnemyPos"].length; i++) {
				...
				let str1 = worldStr.slice(
					0,
					xPos - Math.floor(generalArt.enemy.length / 2)
				); // before enemy
				let str2 = worldStr.slice(
					xPos + 1 + Math.floor(generalArt.enemy.length / 2)
